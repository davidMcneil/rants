...
fn test_decoder_simple() {
    ...
    buf.put("ping\r\npOnG\r\n");
    assert_eq!(codec.decode(buf).unwrap().unwrap().unwrap(),
               ServerMessage::Ping);
    assert_eq!(codec.decode(buf).unwrap().unwrap().unwrap(),
               ServerMessage::Pong);
...
fn test_decoder_info() {
    ...
    buf.put("INFO {\"server_id\":\"Zk0GQ3JBSrg3oyxCRRlE09\",\"version\":\"1.2.0\",\"proto\":1,\"\
...
fn test_decoder_msg() {
    ...
    buf.put("orld!\r\n");
    assert_eq!(codec.decode(buf).unwrap().unwrap().unwrap(),
               ServerMessage::Msg(Msg::new(Subject::from_str("test").unwrap(),
                                           String::from("0"),
                                           None,
